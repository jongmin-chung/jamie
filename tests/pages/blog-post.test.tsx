...
<p>이렇게 사용할 수 있습니다.</p>`);
  ...
  it('should render post content as HTML', async () => {
    ...
    expect(screen.getByText(/React Hooks는 React 16.8에서 소개된/)).toBeInTheDocument();
  });
  ...
  it('should render code blocks properly', async () => {
    ...
    const codeBlock = screen.getByText(/const \[count, setCount\] = useState\(0\);/);
  ...
  it('should have back to blog listing link', async () => {
    ...
    const backLink = screen.getByRole('link', { name: /블로그 목록|Back to Blog/ });
  ...
  it('should handle Korean headings and content', async () => {
    ...
    const koreanHeading = screen.getByRole('heading', { name: /React Hooks 완전 가이드/ });
