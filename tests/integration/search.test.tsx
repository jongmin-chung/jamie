...
const mockSearchIndex = {
  documents: [
    ...
    },
    {
      ...
      title: 'React 성능 최적화',
      content:
        'React 애플리케이션의 성능을 향상시키는 다양한 기법들을 알아봅시다.',
      category: '프론트엔드',
...
describe('Search Functionality Integration', () => {
  ...
  test('should clear results when search input is cleared', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText('React Hooks 완벽 가이드')
  ...
  test('should display search result metadata', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('TypeScript 기초부터 심화까지')
      ).toBeInTheDocument()
      // Should show category or other metadata
      expect(
        screen.getByText(/프론트엔드|JavaScript|TypeScript/)
      ).toBeInTheDocument()
    })
  })
