...
jest.mock('@/lib/search', () => ({
  initializeSearch: jest.fn(() => ({
    search: jest.fn((query: string) => {
      const mockResults = [
        { id: '1', title: 'React Hooks 완전 가이드', slug: 'react-hooks-guide' },
        { id: '2', title: 'TypeScript 기초', slug: 'typescript-basics' },
        { id: '3', title: '리액트 성능 최적화', slug: 'react-performance' }
      ];

      if (query === 'React') {
        return mockResults.filter(r => r.title.includes('React') || r.title.includes('리액트'));
      }
      if (query === '리액트') {
...
describe('SearchBox Component Unit Tests', () => {
  ...
  test('should clear results when search input is cleared', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('React Hooks 완전 가이드')).not.toBeInTheDocument();
    });
  });
