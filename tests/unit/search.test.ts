  ...
  documents: [
    {
      ...
      content:
        'React Hooks는 함수형 컴포넌트에서 상태 관리를 가능하게 합니다. useState, useEffect 등이 있습니다.',
      category: '프론트엔드',
    ...
    },
    {
      ...
      content:
        'TypeScript는 JavaScript에 정적 타입을 추가한 언어입니다. 타입 안전성을 제공합니다.',
      category: '프론트엔드',
    ...
    },
    {
      ...
      title: '리액트 성능 최적화 기법',
      content:
        '리액트 애플리케이션의 성능을 향상시키는 다양한 방법들을 알아봅시다. 메모이제이션, 코드 스플리팅 등이 있습니다.',
      category: '프론트엔드',
    ...
    },
    {
      ...
      content:
        'Node.js를 사용한 백엔드 API 개발 방법을 설명합니다. Express.js 프레임워크를 활용합니다.',
      category: '백엔드',
...
describe('Search Utilities Unit Tests', () => {
  ...
  describe('search functionality', () => {
    ...
    test('should search by title in Korean', () => {
      ...
      expect(results.length).toBeGreaterThan(0)
      const titles = results.map((r: any) => r.title)
      expect(titles.some((title: string) => title.includes('리액트'))).toBe(
        true
      )
    })
    ...
    test('should search by tags', () => {
      ...
      expect(results.length).toBeGreaterThan(0)
      const hasJavaScriptTag = results.some(
        (r: any) => r.tags && r.tags.includes('JavaScript')
      )
    ...
    test('should handle special characters in search', () => {
      ...
      expect(results.length).toBeGreaterThan(0)
      const foundNodeJS = results.some((r: any) => r.title.includes('Node.js'))
  ...
  describe('search result structure', () => {
    ...
    test('should maintain original document data', () => {
      ...
      const originalDoc = mockSearchIndex.documents.find(
        (doc) => doc.id === firstResult.id
      )
  ...
  describe('Korean language support', () => {
    ...
    test('should handle mixed Korean and English', () => {
      ...
      expect(results.length).toBeGreaterThan(0)
      const foundGuide = results.some((r: any) => r.title.includes('가이드'))
      expect(foundGuide).toBe(true)
    })
