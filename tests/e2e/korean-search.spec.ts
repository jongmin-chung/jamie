...
test.describe('Korean Search Functionality E2E Tests', () => {
  ...
  test('should search Korean terms successfully', async ({ page }) => {
    ...
    for (const term of koreanSearchTerms) {
      ...
      const hasNoResults = await page.locator('text=/검색 결과가 없습니다|결과 없음/').count() > 0;
  ...
  test('should handle Korean character composition correctly', async ({ page }) => {
    const searchInput = page.getByPlaceholder(/검색/);
  ...
  test('should handle Korean punctuation and special characters', async ({ page }) => {
    const searchInput = page.getByPlaceholder(/검색/);
  ...
  test('should provide relevant Korean search suggestions', async ({ page }) => {
    const searchInput = page.getByPlaceholder(/검색/);
  ...
  test('should maintain Korean search history and context', async ({ page }) => {
    const searchInput = page.getByPlaceholder(/검색/);
  ...
  test('should handle Korean search across different categories', async ({ page }) => {
    const searchInput = page.getByPlaceholder(/검색/);
  ...
  test('should provide Korean error messages and feedback', async ({ page }) => {
    const searchInput = page.getByPlaceholder(/검색/);
    
    // Search for terms that likely have no results
    const noResultsTerms = ['xyz123가나다', '존재하지않는검색어', 'nonexistent한글term'];
    ...
    for (const term of noResultsTerms) {
      ...
      const noResultsMessage = page.locator('text=/검색 결과가 없습니다|결과가 없습니다|검색된 글이 없습니다/');
