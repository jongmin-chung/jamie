...
test.describe('Mobile Responsiveness E2E Tests', () => {
  ...
  test('should display blog post content properly on mobile', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
  ...
  test('should handle mobile table of contents', async ({ page }) => {
    ...
    const desktopTOC = page.locator('[data-testid="table-of-contents"]:not([data-testid="mobile-table-of-contents"])');
  ...
  test('should display Korean text properly on mobile devices', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto('/');

    // Korean text should not be corrupted on mobile
    const koreanTextElements = page.locator('h1, h2, h3, p, a').filter({ hasText: /[ㄱ-ㅎㅏ-ㅣ가-힣]/ });
