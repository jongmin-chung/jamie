...
test.describe('Complete User Journey E2E Tests', () => {
  ...
  test('should complete full user journey from homepage to blog post', async ({ page }) => {
    ...
    const tocExists = await page.locator('[data-testid="table-of-contents"]').count() > 0;
    if (tocExists) {
    ...
    await page.getByRole('link', { name: /블로그|목록|뒤로/ }).first().click();
  ...
  test('should handle error scenarios gracefully', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText(/404|찾을 수 없습니다|페이지가 없습니다/);
    ...
    await expect(page.locator('text=/검색 결과가 없습니다|결과가 없습니다/')).toBeVisible();
  });

  test('should maintain performance standards during user journey', async ({ page }) => {
    ...
    await page.locator('[data-testid="search-results"]').waitFor({ timeout: 1000 });
