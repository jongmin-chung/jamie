...
function analyzeBundle() {
  ...
  try {
    ...
    const buildId = fs
      .readFileSync(path.join(nextDir, 'BUILD_ID'), 'utf8')
      .trim()
    ...
    if (fs.existsSync(staticDir)) {
      ...
      jsChunks.forEach((chunk) => {
        ...
        if (isLarge) {
          hasLargeChunks = true
          console.log(
            `   ⚠️  Chunk exceeds ${PERFORMANCE_THRESHOLDS.MAIN_BUNDLE} KB threshold`
          )
        }
      })
    ...
    items.forEach((item) => {
      ...
      if (item.isDirectory()) {
        ...
        if (stats.size > PERFORMANCE_THRESHOLDS.MAX_IMAGE_SIZE * 1024) {
          hasLargeImages = true
          console.log(
            `🔴 Large image: ${path.relative(process.cwd(), fullPath)} (${sizeKB} KB)`
          )
        }
      }
    })
...
function checkStaticGeneration() {
  ...
  try {
    ...
    if (nextConfig.output === 'export') {
      ...
      warnings.push(
        '⚠️  Static export not enabled - consider for better performance'
      )
    }
    ...
    if (nextConfig.images) {
      ...
      if (
        nextConfig.images.formats &&
        nextConfig.images.formats.includes('image/webp')
      ) {
...
`
  ...
  fs.writeFileSync(
    path.join(process.cwd(), 'scripts/lighthouse-audit.sh'),
    lighthouseScript
  )
  fs.chmodSync(path.join(process.cwd(), 'scripts/lighthouse-audit.sh'), '755')

  console.log(
    '✅ Lighthouse audit script created at scripts/lighthouse-audit.sh'
  )
}
...
function main() {
  ...
  console.log(
    `Bundle Analysis: ${results.bundle ? '✅ PASS' : '❌ NEEDS IMPROVEMENT'}`
  )
  console.log(
    `Image Optimization: ${results.images ? '✅ PASS' : '❌ NEEDS IMPROVEMENT'}`
  )
