  ...
  const optimizations = [
    {
      ...
      check: () => currentConfig.includes('images:') && !currentConfig.includes('unoptimized: true'),
      recommendation: 'Enable Next.js image optimization for better Core Web Vitals'
    },
    {
      name: 'Static export ready',
      check: () => currentConfig.includes('output:') && currentConfig.includes('export'),
      recommendation: 'Configure for static export to maximize performance'
    },
    {
      name: 'Compression enabled',
      check: () => currentConfig.includes('compress:') || currentConfig.includes('gzip'),
    ...
    },
    {
      ...
      check: () => currentConfig.includes('swcMinify:') || !currentConfig.includes('minify'),
...
function validateStaticGeneration() {
  ...
  try {
    ...
    console.log(`🔍 Search documents: ${Array.isArray(searchIndex) ? searchIndex.length : 'Invalid format'}`);
    
    // Validate Korean content
    const koreanPosts = postsMetadata.filter(post => 
      /[ㄱ-ㅎㅏ-ㅣ가-힣]/.test(post.title + post.excerpt)
    );
    console.log(`🇰🇷 Posts with Korean content: ${koreanPosts.length}/${postsMetadata.length}`);
  ...
  try {
    ...
    if (fs.existsSync(nextConfigPath)) {
      ...
      if (config.output === 'export') {
        ...
        console.log(`✅ Static export completed in ${(exportTime / 1000).toFixed(1)}s`);
...
`;
  ...
  fs.writeFileSync(path.join(process.cwd(), 'scripts/build-optimized.sh'), buildScript);
...
function main() {
  ...
  console.log(`Environment: ${results.environment ? '✅ PASS' : '❌ NEEDS ATTENTION'}`);
  console.log(`Configuration: ${results.config ? '✅ OPTIMIZED' : '⚠️  NEEDS OPTIMIZATION'}`);
  console.log(`Static Generation: ${results.static ? '✅ VALID' : '❌ ISSUES FOUND'}`);
